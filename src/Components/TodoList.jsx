      ...
      <ul className="bg-gray-50 p-5 rounded-xl flex flex-col gap-5 shadow-xl">
        {data.map((item) => (
          <li className="border-solid border-2 border-slate-200 rounded-md p-2.5" key={item.id}>
            {editId === item.id ? (<div><input type="text" value={editText} onChange={(e) => setEditText(e.target.value)} /> <button onClick={()=>handleSave(item.id)}>Save</button></div>):(<h2 className="font-Sora md:text-lg font-light text-600 text-base">Todo: <span className="font-Sora font-semibold md:text-3xl text-xl">{item.text}</span></h2>)}
