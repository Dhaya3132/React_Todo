      ...
      <ul className="bg-gray-50 p-5 rounded-xl flex flex-col gap-5 shadow-xl">
        {data.map((item) => (
          <li className="border-solid border-2 border-slate-200 rounded-md p-2.5" key={item.id}>
            {editId === item.id ? (<div><input type="text" value={editText} onChange={(e) => setEditText(e.target.value)} className='border-2 border-solid border-slate-200 bg-transparent rounded-xl p-2' /> <button onClick={()=>handleSave(item.id)} className='bg-blue-400 px-5 py-2 text-Sora font-semibold text-white rounded-xl'>Save</button></div>):(<h2 className="font-Sora md:text-lg font-light text-600 text-base">Todo: <span className="font-Sora font-semibold md:text-3xl text-xl">{item.text}</span></h2>)}
